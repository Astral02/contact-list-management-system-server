...
exports.getUser = function (req, res, next) {
    ...
    if (req.usertype && req.usertype === 'admin'){
        ContactList.find({username: req.params.id}, function (err, data) {
            if (err) return res.status(400).json(result.error);
            var resData = {
...
exports.createUser = function (req, res, next) {
    ...
    ContactList.find(query, 'username usertype enabled', function (err, data) {
        ...
        ContactList.create(createData, function (errr, datar) {
...
exports.editUser = function (req, res, next) {
    ...
    ContactList.find(query, 'username usertype enabled', function (err, data) {
        ...
        if (data.length === 1) {
            ...
            ContactList.update({_id: req.params.id}, {$set: editData}, function (errr, datar) {
            if (errr) return res.status(400).json(result.error);
...
exports.deleteUser = function (req, res, next) {
    ...
    ContactList.find(query, 'username usertype enabled', function (err, data) {
        ...
        if (data.length > 1) return res.status(400).json(result.auth);
        if (data.length === 1) {
            ContactList.remove({_id: req.params.id}, function (errr, datar) {
                if (errr) return res.status(400).json(result.error);
        ...
        if (data.length === 1) {
            if (data[0].enabled === true && (data[0].usertype === 'user' || data[0].usertype === 'admin')){
                ...
                if (!ucookie) {
                    ...
                    ContactList.update(query, {$set:{login:{ucookie: randomNumber, ucreated: Date.now()}}}, function(errup, dataup){
...
exports.loginOutUser = function (req, res, next) {
    ...
    ContactList.find(query, 'username usertype enabled', function (err, data) {
        ...
        if (data.length > 1) return res.status(400).json(result.auth);
        if (data.length === 1) {
            ContactList.update(query, {$set:{login: { ucookie: null, ucreated: Date.now()}}}, function(errup, dataup){
                if (errup || dataup.n == 0 || data.nModified == 0) return res.status(401).json(result.auth);
                return res.status(200).json(result.success);
            });
        }
    ...
    ContactList.find(query, 'username usertype enabled', function (err, data) {
        ...
        if (data.length === 1) {
            if (data[0].enabled === true && (data[0].usertype === 'admin')){
                ...
                if (!acookie) {
                    var randomNumber = Math.random().toString();
                    ContactList.update(query, {$set:{login:{acookie: randomNumber, acreated: Date.now()}}}, function(errup, dataup){
                        if (errup  || dataup.n == 0 || data.nModified == 0) return res.status(401).json(result.auth);
                        var d = new Date((new Date().getTime() + 60000));
...
exports.loginOutAdmin = function (req, res, next) {
    ...
    ContactList.find(query, 'username usertype enabled', function (err, data) {
        ...
        if (data.length > 1) return res.status(400).json(result.auth);
        if (data.length === 1) {
            ContactList.update(query, {$set:{login: { acookie: null, acreated: Date.now()}}}, function(errup, dataup){
                if (errup || dataup.n == 0 || data.nModified == 0) return res.status(401).json(result.auth);
                return res.status(200).json(result.success);
            });
        }
